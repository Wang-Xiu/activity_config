...
export default function VersionInfo({ versionInfo, isLoading = false, className = '' }: VersionInfoProps) {
    // 格式化时间显示
    ...
    return (
        <div className={`bg-white border border-gray-200 rounded-lg p-4 ${className}`}>
            <div className="flex items-center justify-between mb-3">
                ...
                {versionInfo?.version && (
                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getVersionStatusColor(versionInfo.version)}`}>
